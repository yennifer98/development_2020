...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        if self.prizeMoney>=VOWEL_COST:
            canguess = [a for a in LETTERS if a not in guessed]
        else:
            canguess = [a for a in LETTERS if (a not in guessed) and (a not in VOWELS)]
        return canguess
    def getMove(self, category, obscuredPhrase, guessed):
        canguess = self.getPossibleLetters(guessed)
        flag = self.smartCoinFlip()
        if canguess==[]:
            return 'pass'
        elif flag==True:
